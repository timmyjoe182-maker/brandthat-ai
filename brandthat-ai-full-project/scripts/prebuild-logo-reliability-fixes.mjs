...
`,
  "bounded timeout and image storage helpers"
);
...
logo = replaceOnce(logo, oldSoftware, newSoftware, "distinct software direction fields");

const oldGeneric = `    [names[0], "wordmark-led identity with one ownable detail from the brand meaning", visual || \`a restrained \${label} cue integrated into the typography\`, type || "clean readable type matched to the brand personality", "website headers, social profile, and launch materials", "This direction makes the name the strongest asset and keeps the mark easy to use. It is designed for clarity before decoration.", "The custom type detail can simplify when the mark is used small.", categoryPalette || "the palette should support contrast, recognition, and practical use."],
...
logo = replaceOnce(logo, oldGeneric, newGeneric, "distinct generic direction fields");
...
if (changed) {
  console.log("Applied logo reliability prebuild fixes.");
} else {
  console.log("Logo reliability prebuild fixes already present.");
}
